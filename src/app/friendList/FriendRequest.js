...
<Button className={`bg-blue-600 cursor-pointer mb-2`}  onClick = {()=>{}} size={`lg`} >
  <UserPlus className=' h-4 w-4'/><span>Confirm</span>
</Button>

<Button className={`bg-blue-600 cursor-pointer`}  onClick = {()=>{}} size={`lg`} >
  <UserMinus className='mr-2 h-4 w-4'/><span>Delete</span>
</Button>
</div>
      </motion.div>
    </AnimatePresence>
